...
class DBConnection:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"Supabase client initializing with URL: {supabase_url} and key type: {key_type}")
            
            if not supabase_url or not supabase_key:
                logger.error("CRITICAL: Supabase URL or Key is missing in configuration. Cannot initialize client.")
            ...
            logger.info("Supabase AsyncClient created successfully.")
            self._initialized = True
            logger.debug(f"Database connection initialized with Supabase using {key_type}")
        except Exception as e:
            logger.error(f"CRITICAL: Failed to create Supabase AsyncClient. Error: {e}")
