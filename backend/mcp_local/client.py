...
# --- INICIO DE LA SOLUCIÓN AUTOMATIZADA ---

@asynccontextmanager
async def streamablehttp_client(url: str):
    """
    Reemplazo para el 'streamablehttp_client' que faltaba.
    Utiliza WebSockets (que sí está instalado) para crear la conexión.
    """
    # Convierte la URL http:// o https:// a ws:// o wss:// para la conexión WebSocket
    wss_url = url.replace("https://", "wss://").replace("http://", "ws://")
    try:
        async with websockets.connect(wss_url) as websocket:
            # El objeto websocket puede funcionar como el stream de lectura y escritura
            # que la librería MCP necesita para funcionar.
            yield websocket, websocket, None
    except Exception as e:
        # Reutilizamos el logger que ya existe en este fichero para ver errores
        logger.error(f"Conexión WebSocket a {wss_url} falló: {e}")
        raise

# --- FIN DE LA SOLUCIÓN AUTOMATIZADA ---
